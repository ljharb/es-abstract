...
module.exports = function (t, year, actual, extras) {
	...
	t.test('ArrayBuffers supported', { skip: typeof ArrayBuffer !== 'function' }, function (st) {
		...
		forEach(v.nonBooleans, function (nonBoolean) {
			...
			if (year >= 2017) {
				st['throws'](
					function () { GetValueFromBuffer(new ArrayBuffer(8), 0, int8, false, 'Unordered', nonBoolean, true, order); },
					TypeError,
			...
			st.test(name + ': ' + debug(testCase.value), function (s2t) {
				forEach(esV.getNumberTypes(year), function (type) {
					...
					*/
					...
					s2t.equal(
						GetValueFromBuffer(view.buffer, 0, typeForYear, true, order, true),
						littleVal,
						'buffer with type ' + typeForYear + ', little -> little, yields expected value'
					);

					if (hasBigEndian) {
						s2t.equal(
							GetValueFromBuffer(view.buffer, 0, typeForYear, true, order, false),
							view['get' + method](0, false),
							'buffer with type ' + typeForYear + ', little -> big, yields expected value'
						);
						...
						s2t.equal(
							GetValueFromBuffer(view.buffer, 0, typeForYear, true, order, false),
							bigVal,
							'buffer with type ' + typeForYear + ', big -> big, yields expected value'
						);

						s2t.equal(
							GetValueFromBuffer(view.buffer, 0, typeForYear, true, order, true),
							view['get' + method](0, true),
							'buffer with type ' + typeForYear + ', big -> little, yields expected value'
						);
					}
				});
