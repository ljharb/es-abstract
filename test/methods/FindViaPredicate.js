...
module.exports = function (t, year, FindViaPredicate) {
	...
	forEach(v.primitives, function (primitive) {
		forEach([].concat(
			year <= 2024 ? ['ascending', Enum('ascending')] : [],
			year >= 2024 ? ['ASCENDING', Enum('ASCENDING')] : []
		), function (direction) {
			t['throws'](
				function () { FindViaPredicate(primitive, 0, direction, function () {}); },
				TypeError,
	...
	forEach(v.notNonNegativeIntegers, function (notNonNegativeInteger) {
		forEach([].concat(
			...
			t['throws'](
				function () { FindViaPredicate({}, notNonNegativeInteger, direction, function () {}); },
				TypeError,
	...
	forEach(v.nonFunctions, function (nonFunction) {
		forEach([].concat(
			...
			t['throws'](
				function () { FindViaPredicate({}, 0, direction, nonFunction); },
				TypeError,
	...
	forEach([].concat(
		...
		t.test(direction, function (st) {
			var expectedIndex = 1;
			st.plan(((expectedIndex + 1) * 3) + 1);

			var result = FindViaPredicate(arr, fakeLength, direction, function (element, index, obj) {
	...
	forEach([].concat(
		year <= 2024 ? ['descending', Enum('descending')] : [],
		year >= 2024 ? ['DESCENDING', Enum('DESCENDING')] : []
	), function (direction) {
		t.test(direction, function (st) {
			var expectedIndex = 3;
	...
	forEach([].concat(
		...
		t.test(direction + ': not found', function (st) {
