...
test('Decode', function (t) {
	forEach(v.nonStrings, function (nonString) {
		...
		t['throws'](
			function () { ES.Encode('', nonString); },
			TypeError,
			'reservedSet: ' + debug(nonString) + ' is not a String'
		);
	});

	t.equal(ES.Decode('%F0%9F%92%A9', ''), '💩', 'surrogate pairs are decoded as expected');
	t.equal(ES.Decode('a%62c%5a', 'ac'), 'abcZ', 'reservedSet is respected; hex digits are handled regardless of case');
