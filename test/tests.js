...
var twoSixtyFour = hasBigInts && !!safeBigInt && safeBigInt(Math.pow(2, 64));
var twoSixtyThree = hasBigInts && !!safeBigInt && safeBigInt(Math.pow(2, 63));

var clearBuffer = require('./helpers/clearBuffer');

/**
 * @template T
 * @param {{ SetValueInBuffer: T & Function }} ES
 * @param {tape.Test} st
 * @param {typeof bufferTestCases[keyof typeof bufferTestCases]} testCase
 * @param {boolean} isTypedArray
 * @param {Parameters<T>[5]} order
 */
var testSetValueInBuffer = function (ES, st, testCase, isTypedArray, order) {
	/** @param {import('../types').TypedArrayTypeByName<import('../types').TypedArrayName>} type */
	return function (type) {
		var isBigInt = esV.isBigIntTAType(type);
		var Z = isBigInt ? safeBigInt : Number;
		var elementSize = esV.elementSizes[/** @type {`$${Exclude<typeof type, 'Uint8C'> | 'Uint8Clamped'}Array`} */ ('$' + (type === 'Uint8C' ? 'Uint8Clamped' : type) + 'Array')];
		...
		*/
		...
		var resultObj = result[type === 'Float64' ? 'setAsLittle' : 'setAsTruncatedLittle'];
		st.deepEqual(
			Array.prototype.slice.call(copyBytes, 0, elementSize),
			Array.prototype.slice.call(new Uint8Array(resultObj.bytes), 0, elementSize),
		...
		if (hasBigEndian) {
			...
			var bigResultObj = result[type === 'Float64' ? 'setAsBig' : 'setAsTruncatedBig'];
			st.deepEqual(
				Array.prototype.slice.call(copyBytes, 0, elementSize),
				Array.prototype.slice.call(new Uint8Array(bigResultObj.bytes), 0, elementSize),
...
/** @type {(ES: import('../types').ES, skips?: Record<string, boolean>) => import('./testHelpers').TapeLike} */
var makeTest = function makeTest(ES, skips) {
	/** @type {import('./testHelpers').TapeLike} */
	var test = function test(opName, maybeOpts) {
		var origOpts = arguments.length > 2 ? maybeOpts : { __proto__: null };
		/** @type {tape.TestOptions} */
		var opts = assign(
			{},
			origOpts,
			{ skip: (skips && skips[opName]) || origOpts.skip }
		);
		var cb = /** @type {tape.TestCase} */ (arguments.length > 2 ? arguments[2] : maybeOpts);
		return tape(
			opName,
			opts,
			ES[opName] && typeof cb === 'function' ? /** @type {tape.TestCase} */ function (t) {
...
/** @type {<T, C extends import('../types').BasicConstructor>(speciesConstructor?: import('../types').Constructor<T, C> | null | undefined) => import('../types').Constructor<T, C>} */
var getArraySubclassWithSpeciesConstructor = function getArraySubclass(speciesConstructor) {
	/**
	 * @constructor
	 * @type {import('../types').Constructor<typeof Bar, ArrayConstructor>}
	 * */
	function Bar() {
		/** @type {unknown[]} */
		var inst = [];
		Object.setPrototypeOf(inst, Bar.prototype);
		// @ts-expect-error arrays do have .constructor
...
/** @type {(t: tape.Test, enumerableOwnNames: (O: object) => string[]) => object} */
...
/** @type {(t: tape.Test, ES: object, StringToNumber: (x: string) => number) => void} */
...
/** @type {(t: tape.Test, ES: { ToPrimitive: (o: object) => import('../types').primitive }, ToNumber: (x: unknown) => number) => void} */
...
/** @type {import('./testHelpers').EditionTests<import('../es5')>} */
var es5 = function ES5(ES, ops, expectedMissing, skips) {
	...
	test('ToPrimitive', function (t) {
		t.test('primitives', function (st) {
			/** @type {(primitive: import('../types').primitive) => void} */
	...
	test('ToNumber', function (t) {
		...
		t.test('objects', function (st) {
			forEach(v.objects, function (object) {
				st.equal(
					ES.ToNumber(object),
					ES.ToNumber(/** @type {Exclude<ReturnType<typeof ES.ToPrimitive, 'bigint'>>} */ (ES.ToPrimitive(object))),
...
/** @type {import('./testHelpers').EditionTests<import('../es2015')>} */
var es2015 = function ES2015(ES, ops, expectedMissing, skips) {
	...
	test('AdvanceStringIndex', function (t) {
		...
		forEach(esV.notInts, function (nonInt) {
			t['throws'](
				// @ts-expect-error
				function () { ES.AdvanceStringIndex('abc', nonInt, false); },
				TypeError,
	...
	test('ArraySetLength', function (t) {
		...
		t.test('making length nonwritable', { skip: !getOwnPropertyDescriptor }, function (st) {
			...
			st.deepEqual(
				/** @type {NonNullable<typeof getOwnPropertyDescriptor>} */ (getOwnPropertyDescriptor)(a, 'length'),
				{
					configurable: false,
					enumerable: false,
					value: 1,
	...
	test('ArraySpeciesCreate', function (t) {
		...
		t.test('works with species construtor', { skip: !hasSpecies }, function (st) {
			...
			/**
			 * @constructor
			 * @type {import('../types').Constructor<Foo, [len: import('../types').integer]>}
			 */
			function Foo(len) {
				this.length = len;
		...
		t.test('throws with object non-constructor species constructor', { skip: !hasSpecies }, function (st) {
			forEach(v.objects, function (obj) {
	...
	test('Call', function (t) {
		...
		ES.Call(
			/** @type {(this: unknown, a: number, b: number, ...args: number[]) => void} */
			function (a, b) {
		...
		t.test('Call doesn’t use func.apply', function (st) {
			...
			/** @type {(this: typeof receiver, a: 1, b: 2, ...args: number[]) => void})} */
	...
	test('CompletionRecord', function (t) {
		...
		forEach(/** @type {const} */ (['normal', 'break', 'continue', 'return']), function (nonAbruptType) {
			var completion = new ES.CompletionRecord(nonAbruptType, sentinel);
			t.equal(completion['?'](), sentinel, '? returns the value of a non-abrupt completion');
		});
		var throwCompletion = new ES.CompletionRecord('throw', sentinel);
		t['throws'](
		...
		var normalCompletion = new ES.CompletionRecord('normal', sentinel);
		...
		forEach(/** @type {const} */ (['break', 'continue', 'return', 'throw']), function (nonNormalType) {
			var completion = new ES.CompletionRecord(nonNormalType);
			t['throws'](
				function () { completion['!'](); },
				SyntaxError,
	...
	test('CreateDataProperty', function (t) {
		...
		forEach(v.propertyKeys, function (propertyKey) {
			/** @type {Parameters<typeof ES.CreateDataProperty>[0]} */
			var obj = {};
			...
			t.test('with defineProperty', { skip: !$defineProperty }, function (st) {
				var nonWritable = defineProperty(
					/** @type {Parameters<typeof defineProperty>[0]} */ ({}),
					propertyKey,
	...
	test('CreateDataPropertyOrThrow', function (t) {
		...
		var sentinel = {};
		forEach(v.propertyKeys, function (propertyKey) {
			/** @type {{ [k in typeof propertyKey]?: unknown }} */
			var obj = {};
	...
	test('CreateMethodProperty', function (t) {
		...
		t.test('defines correctly', function (st) {
			...
			st.test('property descriptor', { skip: !getOwnPropertyDescriptor }, function (s2t) {
				if (getOwnPropertyDescriptor) { // TS doesn't understand tape's `skip`
					s2t.deepEqual(
	...
	test('Get', function (t) {
		t['throws'](
			// @ts-expect-error
			function () { return ES.Get('a', 'a'); },
			TypeError,
			'Throws a TypeError if `O` is not an Object'
		);
		t['throws'](
			// @ts-expect-error
			function () { return ES.Get({ 7: 7 }, 7); },
			TypeError,
		...
		t.test('Symbols', { skip: !v.hasSymbols }, function (st) {
			var sym = Symbol('sym');
			/** @type {{ [k: typeof sym]: typeof value }} */
			var obj = {};
			obj[sym] = value;
	...
	test('GetMethod', function (t) {
		...
		t['throws'](
			// @ts-expect-error
			function () { return ES.GetMethod({ 7: 7 }, 7); },
			TypeError,
			'Throws a TypeError if `P` is not a property key'
		);
	...
	test('GetOwnPropertyKeys', function (t) {
		...
		t.test('Symbols', { skip: !v.hasSymbols }, function (st) {
			/** @type {{ [k: PropertyKey]: unknown, [Symbol.iterator]?: true }} */
			var O = { a: 1 };
		...
		t.test('non-enumerable names', { skip: !$defineProperty }, function (st) {
			/** @type {{ [k: PropertyKey]: unknown, [Symbol.iterator]?: unknown }} */
			var O = { a: 1 };
	...
	test('GetV', function (t) {
		...
		t.test('getter observability of the receiver', { skip: !$defineProperty || !Object.isExtensible(Number.prototype) }, function (st) {
			...
			st.teardown(mockProperty(/** @type {Parameters<typeof mockProperty>[0]} */ (/** @type {unknown} */ (Number.prototype)), 'foo', {
				get: function () {
					receivers.push(this);
				}
			}));
				...
				st.test(name + ': ' + debug(testCase.value), function (s2t) {
					forEach(esV.numberTypes, function (type) {
						...
						*/
						...
						var littleVal = /** @type {number} */ (unserialize(result.setAsLittle.asLittle));
						...
						if (hasBigEndian) {
							...
							clearBuffer(view.buffer);
							var bigVal = /** @type {number} */ (unserialize(result.setAsBig.asBig));
	...
	test('IntegerIndexedElementSet', function (t) {
		forEach(v.nonNumbers, function (nonNumber) {
			t['throws'](
				function () { ES.IntegerIndexedElementSet(null, nonNumber, null); },
				TypeError,
				debug(nonNumber) + ' is not a Number'
			);
		});
		...
		forEach([].concat(
			...
			t['throws'](
				function () { ES.IntegerIndexedElementSet(nonTA, 0, null); },
				TypeError,
		...
		t.test('actual typed arrays', { skip: availableTypedArrays.length === 0 }, function (st) {
			if (esV.hasBigInts) {
				forEach(esV.bigIntTypes, function (bigIntType) {
					...
					var ta = new TA(0);
					st['throws'](
						function () { ES.IntegerIndexedElementSet(ta, 0, null); },
						SyntaxError,
			...
			forEach(availableTypedArrays, function (TypedArray) {
				if (!esV.isBigIntTAType(TypedArray)) {
					...
					st.test('can detach', { skip: !esV.canDetach }, function (s2t) {
						...
						s2t['throws'](
							function () { ES.IntegerIndexedElementSet(taD, 0, null); },
							TypeError,
					...
					forEach(v.nonNumbers, function (nonNumber) {
						st['throws'](
							function () { ES.IntegerIndexedElementSet(ta, nonNumber, null); },
							TypeError,
					...
					forEach([].concat(
						...
						if (typeof notNonNegativeInt === 'number') {
							st.equal(
								ES.IntegerIndexedElementSet(ta, notNonNegativeInt, null),
								false,
					...
					st.equal(ta[0], 0, 'is initially zero');
					st.equal(
						ES.IntegerIndexedElementSet(ta, 0, 42),
						true,
						'set is successful'
					);
					st.equal(ta[0], 42, 'has expected value');
				}
			});
	...
	test('InternalizeJSONProperty', function (t) {
		forEach(v.primitives, function (primitive) {
			t['throws'](
				...
				TypeError,
				'holder: ' + debug(primitive) + ' is not an Object'
			);
		});
		...
		forEach(v.nonStrings, function (nonString) {
			t['throws'](
				...
				TypeError,
				'name: ' + debug(nonString) + ' is not a String'
			);
		});
		...
		forEach(v.nonFunctions, function (nonFunction) {
			t['throws'](
				...
				TypeError,
				'reviver: ' + debug(nonFunction) + ' is not a function'
			);
		});

		var passthrough = /** @type {<T>(name: string, val: T) => T} */ function (name, val) { return val; };

		t.deepEqual(
			ES.InternalizeJSONProperty({ a: { b: { c: 1 } } }, 'a', passthrough),
			{ b: { c: 1 } }
		);

		t.deepEqual(
			ES.InternalizeJSONProperty({ a: [{ b: { c: 1 } }, { d: 2 }] }, 'a', passthrough),
			[{ b: { c: 1 } }, { d: 2 }]
		);

		/** @type {(name: string, val: unknown) => unknown} */
	...
	test('IsConcatSpreadable', function (t) {
		...
		t.test('Symbol.isConcatSpreadable', { skip: !hasSymbolConcatSpreadable }, function (st) {
			forEach(v.falsies, function (falsy) {
				/** @type {{ [Symbol.isConcatSpreadable]?: unknown }} */
				var obj = {};
	...
	test('IsPropertyKey', function (t) {
		forEach(/** @type {(number | object)[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			v.numbers,
			v.objects
		)), function (notKey) {
	...
	test('IteratorClose', function (t) {
		...
		t.assertion(
			throwsSentinel,
			function () { ES.IteratorClose({ 'return': function () { throw sentinel; } }, new ES.CompletionRecord('throw', -1)); },
			-1,
		...
		t.assertion(
			throwsSentinel,
			function () { ES.IteratorClose({ 'return': function () { } }, new ES.CompletionRecord('throw', -1)); },
			-1,
	...
	test('NewPromiseCapability', function (t) {
		forEach(/** @type {(typeof v.nonFunctions[number] | typeof v.nonConstructorFunctions[number])[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			v.nonFunctions,
		...
		/**
		 * @constructor
		 * @param {import('../types').PromiseExecutor} executor
		*/
		function C(executor) {
	...
	test('RegExpExec', function (t) {
		...
		t.test('gets and calls a callable "exec"', function (st) {
			var str = '123';
			var o = {
				/** @type {(this: RegExp, S: string) => ReturnType<typeof RegExp.prototype.exec>} */
				exec: function (S) {
	...
	test('RequireObjectCoercible', function (t) {
		...
		t['throws'](
			// @ts-expect-error
			function () { return ES.RequireObjectCoercible(undefined); },
			TypeError,
			'undefined throws'
		);
		t['throws'](
			// @ts-expect-error
			function () { return ES.RequireObjectCoercible(null); },
			TypeError,
			'null throws'
		);
		forEach(/** @type {(object | typeof v.nonNullPrimitives[number])[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			v.objects,
			v.nonNullPrimitives
		)), function (value) {
		...
		t.test('ArrayBuffers supported', { skip: typeof ArrayBuffer !== 'function' }, function (st) {
			...
			forEach(bufferTestCases, function (testCase, name) {
				forEach(esV.numberTypes, function (type) {
					var hasBigEndian = type !== 'Int8' && type !== 'Uint8' && type !== 'Uint8C'; // the 8-bit types are special, they don't have big-endian
					var result = testCase[type === 'Uint8C' ? 'Uint8Clamped' : type];
					var value = /** @type {number} */ (unserialize(testCase.value));
	...
	test('SpeciesConstructor', function (t) {
		...
		t.equal(
			// @ts-expect-error TS has an implicit type somewhere that .constructor is a function
	...
	test('TestIntegrityLevel', function (t) {
		...
		t['throws'](
			// @ts-expect-error
			function () { ES.TestIntegrityLevel({ a: 1 }, 'not sealed or frozen'); },
			/^TypeError: Assertion failed: `level` must be `"sealed"` or `"frozen"`$/,
			'`level` must be `"sealed"` or `"frozen"`'
		);
	...
	test('ValidateAndApplyPropertyDescriptor', function (t) {
		...
		t.test('current is undefined', function (st) {
			...
			st.test('accessor descriptor', { skip: !$defineProperty }, function (s2t) {
				...
				var O = {};
				ES.ValidateAndApplyPropertyDescriptor(undefined, propertyKey, true, accessor, undefined);
				s2t.equal(
					ES.ValidateAndApplyPropertyDescriptor(O, propertyKey, false, accessor, undefined),
					false,
					'when extensible is false, nothing happens'
				);
				s2t.deepEqual(O, {}, 'no changes applied when O is undefined or extensible is false');
				s2t.equal(
					ES.ValidateAndApplyPropertyDescriptor(O, propertyKey, true, accessor, undefined),
					true,
					'operation is successful'
				);
				/** @type {{ [k in typeof propertyKey]?: number }} */
				var expected = {};
	...
	test('ValidateTypedArray', function (t) {
		forEach(/** @type {(typeof unknowns[number] | [])[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			esV.unknowns,
			[[]]
		)), function (nonTA) {
...
/** @type {import('./testHelpers').EditionTests<import('../es2016')>} */
var es2016 = function ES2016(ES, ops, expectedMissing, skips) {
	...
	test('SameValueNonNumber', function (t) {
		...
		forEach(willThrow, function (nums) {
			t['throws'](
				// @ts-expect-error
				function () { return ES.SameValueNonNumber.apply(ES, nums); },
				TypeError,
				'value must be same type and non-number'
			);
		});

		forEach(/** @type {(typeof v.objects[number] | typeof v.nonNumberPrimitives[number])[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			v.objects,
...
/** @type {import('./testHelpers').EditionTests<import('../es2017')>} */
var es2017 = function ES2017(ES, ops, expectedMissing, skips) {
				...
				st.test(name + ': ' + debug(testCase.value), function (s2t) {
					forEach(esV.numberTypes, function (type) {
						...
						*/
						...
						view[/** @type {`set${typeof method}`} */ ('set' + method)](0, esV.isBigIntTAType(type) ? safeBigInt(littleVal) : littleVal, true);
						...
						if (hasBigEndian) {
							...
							view[/** @type {`set${typeof method}`} */ ('set' + method)](0, esV.isBigIntTAType(type) ? safeBigInt(bigVal) : bigVal, false);
	...
	test('StringGetOwnProperty', function (t) {
		forEach(/** @type {(typeof v.nonStrings[number] | typeof v.strings[number])[]} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			v.nonStrings,
			v.strings
	...
	test('ValidateAtomicAccess', function (t) {
		forEach(/** @type {typeof unknowns} */ ([].concat(
			// @ts-expect-error TS sucks with concat
			esV.unknowns,
			[[]]
		)), function (nonTA) {
...
/** @type {import('./testHelpers').EditionTests<import('../es2018')>} */
var es2018 = function ES2018(ES, ops, expectedMissing, skips) {
	...
	test('GetSubstitution', function (t) {
		...
		var capturesX = [];
		t.equal(
			ES.GetSubstitution('abcdef', 'abcdefghi', 0, capturesX, undefined, 'a>$<foo><z'),
			'a>$<oo><z',
	...
	test('PromiseResolve', function (t) {
		t.test('Promises unsupported', { skip: typeof Promise === 'function' }, function (st) {
			st['throws'](
				function () { ES.PromiseResolve(/** @type {PromiseConstructor} */ (function () {}), null); },
				SyntaxError,
	...
	test('SetFunctionLength', function (t) {
		...
		t.test('no length', { skip: !functionsHaveConfigurableNames }, function (st) {
			...
			// @ts-expect-error configurable nonwritable properties are still deleteable
...
/** @type {import('./testHelpers').EditionTests<import('../es2019')>} */
var es2019 = function ES2019(ES, ops, expectedMissing, skips) {
	...
	test('AddEntriesFromIterable', function (t) {
		...
		t.test('Symbol support', { skip: !v.hasSymbols }, function (st) {
			...
			/** @type {(this: {}, key: number, value: unknown) => void} */
			...
			/** @type {(this: {}, key: number, value: number) => void} */
			var badder = function (key, value) {
				throw new EvalError(String(key + value));
			};
			st['throws'](
	...
	test('FlattenIntoArray', function (t) {
		t.test('no mapper function', function (st) {
			/** @type {(tt: tape.Test, depth: number, expected: unknown[]) => void} */
		...
		t.test('mapper function', function (st) {
			/** @type {(tt: tape.Test, mapper: import('../types').Func, expected: unknown[], thisArg?: unknown) => void} */
			...
			testMapper(
				st,
				/** @type {<T>(this: T, x: number) => [T, number]} */
...
/** @type {import('./testHelpers').EditionTests<import('../es2020')>} */
var es2020 = function ES2020(ES, ops, expectedMissing, skips) {
	...
	test('BigInt::bitwiseOR', { skip: !esV.hasBigInts }, function (t) {
		...
		if ($BigInt) { // TS can't figure out how `skip` works
	...
	test('BigInt::equal', { skip: !esV.hasBigInts }, function (t) {
		forEach(v.nonBigInts, function (nonBigInt) {
			...
			t['throws'](
				function () { ES.BigInt.equal($BigInt(0), nonBigInt); },
				TypeError,
				'y: ' + debug(nonBigInt) + ' is not a BigInt'
			);
		});
				...
				st.test(name + ': ' + debug(testCase.value), function (s2t) {
					forEach(esV.allTypes, function (type) {
						...
						*/
						...
						clearBuffer(view.buffer);
						var littleVal = unserialize(result.setAsLittle.asLittle);
						view[/** @type {`set${typeof method}`} */ ('set' + method)](0, isBigInt ? safeBigInt(littleVal) : littleVal, true);
						...
						if (hasBigEndian) {
							...
							clearBuffer(view.buffer);
							var bigVal = unserialize(result.setAsBig.asBig);
							view[/** @type {`set${typeof method}`} */ ('set' + method)](0, isBigInt ? safeBigInt(bigVal) : bigVal, false);
		...
		t.test('actual typed arrays', { skip: availableTypedArrays.length === 0 }, function (st) {
			forEach(availableTypedArrays, function (TypedArray) {
				...
				st.test('can detach', { skip: !esV.canDetach }, function (s2t) {
					...
					s2t['throws'](
						function () { ES.IntegerIndexedElementSet(taD, 0, Z(0)); },
						TypeError,
						debug(taD) + ' is a detached TypedArray'
					);
				...
				forEach(v.nonNumbers, function (nonNumber) {
					st['throws'](
						function () { ES.IntegerIndexedElementSet(ta, nonNumber, Z(0)); },
						TypeError,
						debug(nonNumber) + ' is not a number, throws'
					);
				});
				...
				forEach([].concat(
					...
					if (typeof notNonNegativeInt === 'number') {
						st.equal(
							ES.IntegerIndexedElementSet(ta, notNonNegativeInt, Z(0)),
							false,
							debug(notNonNegativeInt) + ' is not a non-negative-integer number, returns false'
						);
					}
				...
				st.equal(
					ES.IntegerIndexedElementSet(ta, 0, Z(42)),
					true,
					'set is successful'
				);
...
/** @type {import('./testHelpers').EditionTests<import('../es2021')>} */
var es2021 = function ES2021(ES, ops, expectedMissing, skips) {
	...
	test('GetPromiseResolve', function (t) {
		...
		/** @constructor @type {Parameters<typeof ES.GetPromiseResolve>[0]} */
		function C() {}
		...
		t.test('actual typed arrays', { skip: availableTypedArrays.length === 0 }, function (st) {
			forEach(availableTypedArrays, function (TypedArray) {
				...
				st.test('can detach', { skip: !esV.canDetach }, function (s2t) {
					...
					s2t.equal(
						ES.IntegerIndexedElementSet(taD, 0, Z(0)),
						undefined,
				...
				forEach([].concat(
					...
					if (typeof notNonNegativeInt === 'number') {
						st.equal(
							ES.IntegerIndexedElementSet(ta, notNonNegativeInt, Z(0)),
							undefined,
							debug(notNonNegativeInt) + ' is not a non-negative-integer number, returns undefined'
						);
					}
				});
...
/** @type {import('./testHelpers').EditionTests<import('../es2022')>} */
var es2022 = function ES2022(ES, ops, expectedMissing, skips) {
	...
	test('CreateNonEnumerableDataPropertyOrThrow', function (t) {
		forEach(v.primitives, function (primitive) {
			t['throws'](
				function () { ES.CreateNonEnumerableDataPropertyOrThrow(primitive, 'key', null); },
				TypeError,
				'O must be an Object; ' + debug(primitive) + ' is not one'
			);
		});
		...
		forEach(v.nonPropertyKeys, function (nonPropertyKey) {
			t['throws'](
				function () { ES.CreateNonEnumerableDataPropertyOrThrow({}, nonPropertyKey, null); },
				TypeError,
				debug(nonPropertyKey) + ' is not a Property Key'
			);
		});
	...
	test('GetMatchIndexPair', function (t) {
		forEach(v.nonStrings, function (notString) {
			t['throws'](
				function () { return ES.GetMatchIndexPair(notString, { '[[StartIndex]]': 0, '[[EndIndex]]': 0 }); },
				TypeError,
				debug(notString) + ' is not a string'
			);
		});
		...
		forEach([].concat(
			...
			t['throws'](
				function () { return ES.GetMatchIndexPair('', notMatchRecord, { '[[StartIndex]]': 0, '[[EndIndex]]': 0 }); },
				TypeError,
	...
	test('MakeMatchIndicesIndexPairArray', function (t) {
		forEach(v.nonStrings, function (notString) {
			t['throws'](
				function () { return ES.MakeMatchIndicesIndexPairArray(notString, [], [], false); },
				TypeError,
				'`S`: ' + debug(notString) + ' is not a string'
			);
		});
		...
		forEach(v.nonArrays, function (nonArray) {
			t['throws'](
				function () { return ES.MakeMatchIndicesIndexPairArray('', nonArray, [], false); },
				TypeError,
				'`indices`: ' + debug(nonArray) + ' is not a List'
			);

			t['throws'](
				function () { return ES.MakeMatchIndicesIndexPairArray('', [undefined], nonArray, false); },
				TypeError,
	...
	test('SortIndexedProperties', function (t) {
		...
		forEach(
			[].concat(
				v.nonFunctions,
				/** @type {() => void} */ function () {},
				/** @type {(a: unknown, b: unknown) => 0} */ function f(a, b) { return 0; },
				/** @type {(a: unknown) => 0} */ function (a) { return 0; },
		...
		forEach([].concat(
			/** @type {(a: number, b: number) => 0} */ function (a, b) { return 0; },
			v.arrowFunctions.length > 0 ? [
				/* eslint no-new-func: 1 */
		...
		var o = [1, 3, 2, 0];
		t.equal(
			ES.SortIndexedProperties(o, 3, /** @type {(a: number, b: number) => number} */function (a, b) {
				return a - b;
			}),
			o,
		...
		t.equal(
			ES.SortIndexedProperties(o, 4, /** @type {(a: number, b: number) => number} */ function (a, b) {
				return a - b;
			}),
			o,
			'passed object is returned'
		...
		t.equal(
			ES.SortIndexedProperties(o, 6, /** @type {(a: number, b: number) => number} */function (a, b) {
				return a - b;
			}),
			o,
			'passed object is returned'
...
var es2023 = function ES2023(ES, ops, expectedMissing, skips) {
	...
	test('BigInt::toString', function (t) {
		...
		t.test('BigInts are supported', { skip: !esV.hasBigInts }, function (st) {
			forEach([].concat(
				...
				st['throws'](
					function () { ES.BigInt.toString($BigInt(0), nonIntegerNumber); },
					TypeError,
	...
	test('IsWordChar', function (t) {
		...
		forEach(v.nonBooleans, function (nonBoolean) {
			t['throws'](
				function () { ES.IsWordChar(assign({}, rer, { '[[IgnoreCase]]': nonBoolean }), '', 0); },
				TypeError,
				'[[IgnoreCase]]: ' + debug(nonBoolean) + ' is not a Boolean'
			);

			t['throws'](
				function () { ES.IsWordChar(assign({}, rer, { '[[Unicode]]': nonBoolean }), '', 0); },
				TypeError,
				'[[Unicode]]: ' + debug(nonBoolean) + ' is not a Boolean'
			);
		});
	...
	test('TypedArrayCreateSameType', function (t) {
		t.test('no Typed Array support', { skip: availableTypedArrays.length > 0 }, function (st) {
			...
			forEach(v.nonArrays, function (nonArray) {
				st['throws'](
					function () { ES.TypedArrayCreateSameType(Array, nonArray); },
					SyntaxError,
					'no Typed Array support'
				);
			});
		...
		t.test('ArrayBuffers supported', { skip: typeof ArrayBuffer !== 'function' }, function (st) {
			...
			forEach(esV.invalidTATypes, function (invalidType) {
				st['throws'](
					...
					TypeError,
					'type: ' + debug(invalidType) + ' is not a valid String (or type) value'
				);
			});
			...
			if (esV.hasBigInts) {
				st['throws'](
					function () { ES.SetValueInBuffer(new ArrayBuffer(8), 0, 'INT8', /** @type {NonNullable<typeof $BigInt>} */ ($BigInt)(0), true, order); },
					TypeError,
					debug($BigInt(0)) + ' is not a number, but the given type requires one'
				);
	...
	test('TypedArrayCreateFromConstructor', function (t) {
		...
		t.test('Typed Array support', { skip: availableTypedArrays.length === 0 }, function (st) {
			...
			forEach(availableTypedArrays, function (TypedArray) {
				...
				st.equal(whichTypedArray(taBuffer), TypedArray, 'created a ' + TypedArray);
				st.equal(taBuffer.byteOffset, 8, 'byteOffset is 8');
				var expectedLength = expectedLengths[/** @type {`$${typeof TypedArray}`} */ ('$' + TypedArray)];
				st.equal(
					typedArrayLength(taBuffer),
					expectedLength,
					'created a ' + TypedArray + ' of length ' + expectedLength
				);
